...
<template>
  <nav
    :class="[
      ...
      scrolled
        ? 'bg-gray-900/80 backdrop-blur-md border-gray-800 py-3'
        : 'bg-gray-900 border-gray-800 py-5',
    ]"
  >
    <div class="max-w-7xl mx-auto px-4 md:px-8 flex justify-between items-center">
      <!-- Logo / Brand -->
      <RouterLink to="/" class="flex items-center gap-2 group">
        <img src="/images/Icon.png" alt="" class="w-12 h-12" />
      </RouterLink>

      <!-- Desktop Navigation -->
      <div class="hidden md:flex items-center gap-8">
        <RouterLink
          v-for="route in ['/', '/about', '/projects']"
          :key="route"
          :to="route"
        ...
        >
          ...
          <span
            class="absolute bottom-0 left-0 w-0 h-0.5 bg-accent transition-all duration-300 group-hover:w-full group-[.router-link-active]:w-full"
          ></span>
        </RouterLink>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="hidden md:flex items-center gap-3 border-l border-gray-700 pl-4">
          <button
            @click="onClickSocial('https://www.instagram.com/dimazzbagazz/')"
          ...
          <button
            @click="onClickSocial('https://github.com/Diba15')"
            class="text-gray-400 hover:text-accent transition-colors hover:scale-110 transform duration-200"
          >
            <i :class="`pi pi-github text-lg`"></i>
          </button>
          <button
            @click="onClickSocial('https://www.linkedin.com/in/dimasbagassaputro/')"
            class="text-gray-400 hover:text-accent transition-colors hover:scale-110 transform duration-200"
          >
  ...
  <div
    v-show="mobileOpen"
    class="fixed inset-0 z-[60] bg-gray-900/95 backdrop-blur-xl transition-all duration-300"
  >
